...
# 为什么A的定义与N相同？
atomdefs = {'H': (1.0, 1.00794), 'HO': (1.0, 1.00794), 'C': (6.0, 12.0107), 'A': (7.0, 14.00674), 'N': (7.0, 14.00674),
            'O': (8.0, 15.9994), 'P': (15.0, 30.973761), 'K': (19.0, 39.0983),
...
def main():
    ...
    parser.add_argument("--res", "-R", type=float,
                        help="Resolution in A, equivalent to Gaussian lowpass with 1/e width at 1/res", default=2.8)
    ...
    parser.add_argument("--center", action="store_true", help="Move the atomic center to the center of the box",
                        default=False)
    ...
    parser.add_argument("--full", action="store_true",
                        help="Apply non-crystallographic symmetry to obtain 'full' structure.", default=False)
    parser.add_argument("--quiet", action="store_true", help="Verbose is the default", default=False)
    parser.add_argument("--model", type=int,
                        help="Extract only a single numbered model from a multi-model PDB", default=None)
    parser.add_argument("--ppid", type=int, help="Set the PID of the parent process, used for cross platform PPID",
                        default=-1)
    parser.add_argument("--verbose", "-v", dest="verbose", action="store", metavar="n", type=int,
                        help="verbose level [0-9], higher number means higher level of verboseness", default=0)
    parser.add_argument("--quick", action="store_true",
                        help="Use a slight approximation to the Gaussian during insertion. Does not support B-factors.",
                        default=False)
    parser.add_argument("--addpdbbfactor", action="store_true",
                        ...
                        default=False)
    parser.add_argument("--omit", type=float, help="Randomly omit this percentage of atoms in the output map.",
                        default=0.0)
    ...
    if not options.quick:
        ...
        try:
            boxs = options.box.split(',')
        except:
            pass
        try:
            boxs = [int(i) for i in boxs]
            # 虽然要输入3个值，但是似乎只保留了最大值？
        ...
        for line in infile:
            if options.model != None:
                if line[:5] == "MODEL":
                    if int(line.split()[1]) == options.model:
                        stm = True
            ...
            if line[:5] == "HELIX":
                ...
                if int(line[20:25]) > lhelix or line[19] != lch:
                    ...
                    ihelix = -1
                # ihelix是每一行helix最后一个值减2的累积和、lhelix是最后一行helix倒数第三个值、lch是最后一行helix第二个数值前面的字符
                lch = line[19]
                lhelix = int(line[32:37])

            elif line[:5] == "SHEET":
                # isheet是每一行sheet第三个数值与第四个数值的差值+1的累计和
                isheet += abs(int(line[22:26]) - int(line[33:37])) + 1

            elif line[:4] == 'ATOM' or (line[:6] == 'HETATM' and options.het):
                if lastres != line[17:21]:
                    ires += 1
                    # 赋给lres又没有被用到，因此这个判断永真，ires等于ATOM（可能加上HETATM）的行数
                    lres = line[17:21]

                if chains and not (line[21] in chains):
                    continue

                try:
                    # aseq是最后一行ATOM或HETATM的第一个数值
                    aseq = int(line[6:11].strip())
                except:
                    aseq = -1

                try:
                    # resol是最后一行ATOM或HETATM的第二个数值
                    resol = int(line[22:26].strip())
                except:
                    resol = 10

                try:
                    # a是原子符号、x、y、z分别是第三、四、五个数值
        ...
        print("Atomic center at %1.1f,%1.1f,%1.1f (center of volume at 0,0,0)" % (
            old_div(aavg[0], natm), old_div(aavg[1], natm), old_div(aavg[2], natm)))
        ...
        if options.addpdbbfactor:
            print("WARNING: B-factors not supported in quick mode")
        if options.full:
            print("WARNING: --full not supported in quick mode")
            sys.exit(1)
        outmap = pdb_2_mrc(args[0], options.apix, options.res, options.het, box, chains, options.model, options.center,
                           options.quiet)
...
def pdb_2_mrc(file_name, apix=1.0, res=2.8, het=False, box=None, chains=None, model=None, center=False, quiet=False):
    ...
    try:
        ...
        if isinstance(box, int):
            ...
            spl = box.split(',')
            if len(spl) == 1:
                outbox[0] = outbox[1] = outbox[2] = int(spl[0])
            else:
                outbox[0] = int(spl[0])
                outbox[1] = int(spl[1])
                outbox[2] = int(spl[2])
    ...
    if not quiet:
        print("Box size: %d x %d x %d" % (outbox[0], outbox[1], outbox[2]))

    # initialize the final output volume
    outmap = EMData()
    outmap.set_size(outbox[0], outbox[1], outbox[2])
    outmap.to_zero()
    for i in range(len(aavg)):
        aavg[i] = old_div(aavg[i], float(natm))
    ...
    for i, a in enumerate(atoms):
        ...
        try:
            ...
            if center:
                outmap.insert_scaled_sum(gaus, (old_div((a[1] - aavg[0]), apix) + old_div(outbox[0], 2),
                                                old_div((a[2] - aavg[1]), apix) + old_div(outbox[1], 2),
                                                old_div((a[3] - aavg[2]), apix) + old_div(outbox[2], 2)),
                                         old_div(res, (pi * 12.0 * apix)), elec)
            else:
                outmap.insert_scaled_sum(gaus, (
                    old_div(a[1], apix) + old_div(outbox[0], 2), old_div(a[2], apix) + old_div(outbox[1], 2),
                    old_div(a[3], apix) + old_div(outbox[2], 2)), old_div(res, (pi * 12.0 * apix)), elec)
        except:
            print("Skipping %d '%s'" % (i, a[0]))
    if not quiet:
