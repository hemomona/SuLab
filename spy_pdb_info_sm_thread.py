...
"""
# File       : spy_pdb_info_sm_thread.py
# Time       ：2022/6/15 10:15
# Author     ：Jago
# Email      ：18146856052@163.com
# version    ：python 3.7
# Description：使用讯代理，相比spy_pdb_info_sm，改变改变get_opener方法、crawl_process方法
"""
import csv
import gzip
import os.path
...
# 更新代理列表: {ip+port: error_times, ...}
def update_proxy_pool(init_url, pool_dict=None, bad_proxy='', proxy_count=12):
    if pool_dict is None:
        pool_dict = {}

    url = init_url + str(proxy_count)
    request = urlopen(url)
    addresses = request.read().decode('utf-8').split()

    if addresses[0].find('{') == -1:
        for a in addresses:
            print("add proxy:", a)
            pool_dict[a] = 0
        if bad_proxy in pool_dict:
            print("del proxy:", bad_proxy)
            pool_dict.pop(bad_proxy)
    return pool_dict


# 动态ip每过两分钟会变，该方法重新设置代理并返回opener
def get_opener(proxy_address):
    proxy = {
...
# 写入csv文件
def save_record(line_list):
    with open(save_path, 'a', encoding='utf-8', newline='') as f:  # 解决\xa0报错和空白行
        cw = csv.writer(f)
        cw.writerow(line_list)


# 爬取信息总的过程
def crawl_process(href_url):
    global lost_pdb
    global proxy_pool
    table_format = []

    header['User-Agent'] = random.choice(USER_AGENTS_LIST)
    proxy = random.choice(list(proxy_pool.keys()))
    opener = get_opener(proxy)

    if proxy_pool[proxy] > 3:
        proxy_pool = update_proxy_pool(init_proxy_url, proxy_pool, proxy, 1)
        proxy = random.choice(list(proxy_pool.keys()))
        opener = get_opener(proxy)                              # 更新代理设置
    ...
    try:
        ...
        # 提取数据
        table_format.append(id)
        for v in validation_data:
            table_format.append(v)
        for c in carousel_data.values():
            table_format.append(c)
        for u in upper_data:
            table_format.append(u)
        for b in below_data:
            table_format.append(b)
...
pdb_path = "D:/code/SuLab/data/pdb_thread/"
save_path = "data/all_(0-2]_sm_thread.csv"
# init_proxy_url = "http://api.xdaili.cn/xdaili-api//greatRecharge/getGreatIp?spiderId=79d240d53706400fa0cc274cfe1a149c&orderno=YZ20226150221aZvg6Q&returnType=1&count="
init_proxy_url = "http://api.xdaili.cn/xdaili-api//greatRecharge/getGreatIp?spiderId=79d240d53706400fa0cc274cfe1a149c&orderno=YZ20226168830bFNbf7&returnType=1&count="
...
lost_pdb = []
proxy_pool = update_proxy_pool(init_proxy_url)

if __name__ == '__main__':
    has_pdb = []
    df = pd.read_csv("data/all_(0-2]_sm_thread.csv")
    for index, row in df.iterrows():
        has_pdb.append(row['ID'])

    df = pd.read_csv(csv_path)
    print("***start process***")

    pool = Pool(14)
    # pool.map(crawl_process, href)  # 进程池是一个接一个的传参！！！
    for index, row in df.iterrows():
        if row['ID'] not in has_pdb:
            pool.apply_async(func=crawl_process, args=(row['href'], ), callback=save_record)
    pool.close()
    pool.join()

    print("lost", len(lost_pdb), "rows")
    print(lost_pdb)
